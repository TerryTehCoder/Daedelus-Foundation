...
/datum/self_destruct_profile/self_destruct_light_profile/New()
	. = ..()
	active_spinning_lights = list()
	affected_lights = list()

/datum/self_destruct_profile/self_destruct_light_profile/handle_event(event_type, data = null)
	message_admins(span_adminnotice("Self-destruct Light Profile: Handling event [event_type] with data [data]."))
	switch(event_type)
		if(SD_EVENT_START)
			message_admins(span_adminnotice("Self-destruct Light Profile: SD_EVENT_START - Activating red emergency lights and spinning effect."))
			...
			spawn_spinning_lights() // Activate spinning lights for small light fixtures
		if(SD_EVENT_CANCEL, SD_EVENT_FINAL, SD_EVENT_PAUSE)
			message_admins(span_adminnotice("Self-destruct Light Profile: SD_EVENT_CANCEL/FINAL/PAUSE - Deactivating spinning effect and reverting emergency lights."))
		...
		if(SD_EVENT_RESUME)
			...
			message_admins(span_adminnotice("Self-destruct Light Profile: SD_EVENT_RESUME - Resuming light effects."))
			for(var/obj/machinery/light/L in affected_lights)
				if(istype(L) && !QDELETED(L))
					if(istype(L, /obj/machinery/light))
						...
						active_spinning_lights += new_spin_effect // Track this specific instance for cleanup
			spawn_spinning_lights() // Re-activate spinning lights for small light fixtures
