...
// We need to port Kapu's atom_tool_acts cleanup, but that's out of scope for Computer4 Port.
/obj/machinery/c4_embedded_controller/attackby(mob/living/user, obj/item/tool, list/modifiers)
	if(user.combat_mode)
		return NONE
...
/obj/item/key/embedded_controller
	...
	desc = " There's a small logo on the bow."
	icon_state = "ec_key"
