...
/obj/structure/scp_914/MouseDroppedOn(mob/target, mob/user)
	if(target == user)
...
/obj/structure/scp_914/proc/Activate(mob/user)
	...
	processing = TRUE
	playsound(src, 'sound/scp/scp914/refine.ogg', 75, FALSE, 14)
	user.visible_message(
		span_notice("\The [user] activates \the [src]!"),
		span_notice("You activate \the [src]!"))
	SEND_SIGNAL(src, COMSIG_SCP914_ACTIVATED, user)
	...
	playsound(input_part, 'sound/scp/scp914/door_close.ogg', 50, TRUE, -4)
	playsound(output_part, 'sound/scp/scp914/door_close.ogg', 50, TRUE, -4)
	SEND_SIGNAL(src, COMSIG_SCP914_DOORS_CLOSE)
	...
	for(var/atom/movable/A in get_turf(input_part))
		...
		if(A.anchored)
			continue
		if(ismob(A))
			playsound(src, 'sound/scp/scp914/mob_use.ogg', 100, TRUE, 24)
		upgrade_items += A
		A.forceMove(src)
		current_count += 1
	...
	SEND_SIGNAL(src, COMSIG_SCP914_PROCESSING_START)
	sleep(11 SECONDS)
	...
	processing = FALSE
	SEND_SIGNAL(src, COMSIG_SCP914_PROCESSING_COMPLETE)
	...
	playsound(input_part, 'sound/scp/scp914/door_open.ogg', 50, TRUE, -4)
	playsound(output_part, 'sound/scp/scp914/door_open.ogg', 50, TRUE, -4)
	SEND_SIGNAL(src, COMSIG_SCP914_DOORS_OPEN)
